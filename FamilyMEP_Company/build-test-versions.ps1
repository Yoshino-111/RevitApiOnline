[CmdletBinding()]
param(
    [ValidateRange(2020,2027)][int[]]$Years = @(2020..2027),
    [ValidateSet('Debug','Release')][string]$Configuration = 'Release'
)
$ErrorActionPreference = 'Stop'
$portable = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path
$outputName = if ($Configuration -eq 'Debug') { 'multi-debug' } else { 'multi-release' }
...
foreach ($year in $Years) {
    ...
    foreach ($name in @('FamilyMEP.Plugin', 'FamilyMEP.Release')) {
        ...
        & $dotnet build $project -c $Configuration "-p:RevitVersion=$year" "-p:DependencyFolder=$bin" --configfile $config -o $bin --verbosity quiet *> $log
    ...
    Write-Host "PASS Revit $year $Configuration (compile only; in-Revit testing required)"
}
...
& (Join-Path $portable 'dotnet8\dotnet.exe') build $rendererProject -c $Configuration --configfile $config -o $rendererOutput --verbosity quiet *> $rendererLog
